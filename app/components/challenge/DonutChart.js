...
export default function DonutChart({
  ...
  textColor = colors.white,
  max = 100,
  gap = 25, // New prop for the gap between the circles
}) {
  const animatedValue = React.useRef(new Animated.Value(0)).current;
  const circleRef = React.useRef();
  const inputRef = React.useRef();
  ...
  // Adjust inner circle radius for gap
  const innerCircleRadius = radius - strokeWidth / 2 - gap / 2;
      ...
      >
        <G rotation="-90" origin={`${halfCircle}, ${halfCircle}`}>
          <Circle
            ...
            cy={"50%"}
            stroke={color}
            r={innerCircleRadius} // Adjusted inner circle radius
            strokeWidth={strokeWidth}
            fill={colors.orangeSecondary}
            strokeOpacity={0}
          />
          <AnimatedCircle
            ...
            cy={"50%"}
            stroke={colorOuterStroke}
            r={radius - strokeWidth / 2} // Outer circle radius
            strokeWidth={strokeWidth}
            fill="transparent"
            strokeDasharray={circleCircumference}
            strokeDashoffset={circleCircumference}
